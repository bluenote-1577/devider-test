...
def generate_snp_vectors_for_genomes(bam_file, vcf_file, chromosome):
...
# Reading a YAML file
with open("auto_config_megares_deduplicated_for_spike.yaml", 'r') as file:
...
for instance in yaml_data['instances']:
    ...
    truth_bam = f'data_spike/{arg}_true.bam'
    truth_vcf = f'data_spike/{arg}_true.vcf.gz'
    ref_file = f"data_spike/{arg}.fasta"
...
fig, axs = plt.subplots(1, 3, figsize=(16/2.54, 4/2.54))
eb = 'se'
sns.lineplot(x='Coverage', y='Fraction recovered', data=strat_df, ax=axs[0], hue='algorithm', marker = 'o', errorbar = eb)
sns.lineplot(x='% identity to ref.', y='Fraction recovered', data=strat_df[strat_df['% identity to ref.'] > 97.5], ax=axs[1], hue='algorithm', marker = 'o', errorbar = eb)
sns.lineplot(x='Abundance', y='Fraction recovered', data=strat_df, ax=axs[2], hue='algorithm', marker = 'o', errorbar = eb)
...
for ax in axs.flat:
    ...
    ax.set_ylabel('Fraction recovered (spike-in)')
...
plt.savefig('../figures/amr_SPIKE_AF.svg')
plt.show()
